    ...
    public partial class Login : CCSkinMain
    {
        ...
        private void register_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("用户名应为13位数字学号！");
                    return;
                }

                if (!reg2.IsMatch(password))
                {
                    MessageBox.Show("密码应为4-16位数字、字母或下划线");
                    return;
                }

                if (!reg3.IsMatch(name))
                {
                    MessageBox.Show("姓名应为1-30位汉字、字母或数字");
                    return;
                }
        ...
        private void loginExit(object sender, FormClosingEventArgs e)
        {
            if (isClosing)
            {
                if (MessageBox.Show("确认要退出程序吗？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    e.Cancel = false;
